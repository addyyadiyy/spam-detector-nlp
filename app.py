...
# ══════════════════════════════════════════
    ...
    viz_list = [
        ...
        ("Visualization 4 - Top 20 Words.png",
         ...
         "Overall word frequency after preprocessing."),

        ("Visualization 5 - Model Accuracy Comparison.png",
         "Figure 5: Model Accuracy Comparison",
         "BoW + Naive Bayes achieves the highest accuracy."),
    ...
    for i in range(0, len(viz_list), 2):
        ...
        with col1:
            fname, title, caption = viz_list[i]

            st.markdown(
                f"<h4 style='color:#7C3D8F; text-align:center;'>{title}</h4>",
                unsafe_allow_html=True
            )

            st.image(
                f"visualizations/{fname}",
                width=350  # smaller image
            )
        ...
        if i + 1 < len(viz_list):
            with col2:
                fname, title, caption = viz_list[i + 1]
