...
# ══════════════════════════════════════════
    ...
    st.markdown("""
    <div class='hero-banner'>
        <h2 style='margin:0;'>📈 Dataset & Model Visualizations</h2>
        <p style='margin-top:10px;color:#FFD6E7;'>
            Explore dataset characteristics and model performance results.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i in range(0, len(viz_list), 2):
        ...
        with col1:
            ...
            st.image(
                f"visualizations/{fname}",
                use_container_width=True
            )

            st.markdown(
                f"<h5 style='text-align:center;color:#7C3D8F;'>{title}</h5>",
                unsafe_allow_html=True
            )
